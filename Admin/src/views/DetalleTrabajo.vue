...
// ── Estado de carga ─────────────────────────────────────────────────────────
const loadingEval  = ref(true);
const loadingAlumnos = ref(true);
const errorEval    = ref<string | null>(null);
...
const trabajoActual = ref<Evaluacion | null>(null);
const alumnos = ref<Alumno[]>([]);

// Cargar evaluación
onMounted(async () => {
  try {
    const res  = await fetch(`http://localhost:3001/api/evaluaciones/${idTrabajo.value}`);
    const json = await res.json();
    if (!res.ok) throw new Error(json.message);
    trabajoActual.value = json.data;

    // Inicializar mapa de notas para cada alumno
    alumnos.value.forEach((a) => { notasMap.value[a.id] = {}; });
  } catch (err: unknown) {
    errorEval.value = (err instanceof Error ? err.message : null) || "Error al cargar la evaluación";
  } finally {
    loadingEval.value = false;
  }
});

// Cargar alumnos del grupo de la evaluación
onMounted(async () => {
  // Esperar un tick para tener grupo_id
  await new Promise(r => setTimeout(r, 300));
  try {
    // Re-fetch la eval si aún no está lista para obtener grupo_id
    let grupoId = trabajoActual.value?.grupo_id;
    if (!grupoId) {
      const res  = await fetch(`http://localhost:3001/api/evaluaciones/${idTrabajo.value}`);
      const json = await res.json();
      grupoId = json.data?.grupo_id;
    }
    if (!grupoId) throw new Error("No se pudo determinar el grupo");

    const res  = await fetch(`http://localhost:3001/api/grupos/${grupoId}/enrolados`);
    const json = await res.json();
    if (!res.ok) throw new Error(json.message);
    alumnos.value = json.data;
    // Inicializar notas
    json.data.forEach((a: Alumno) => { notasMap.value[a.id] = {}; });
  } catch (err: unknown) {
    errorAlumnos.value = (err instanceof Error ? err.message : null) || "Error al cargar alumnos";
  } finally {
...
const getNotaFinalAlumno = (alumnoId: string) => {
  if (!trabajoActual.value) return "0.0";
  let acumulado = 0;
  trabajoActual.value.criterios.forEach((c) => {
    const calif = notasMap.value[alumnoId]?.[c.id] || 0;
...
const observacionesMap = ref<Record<string, string>>({});

const abrirModalObservacion = (alumno: Alumno) => {
...
<template>
  <admin-layout>
    <div class="p-4 sm:p-6 lg:p-8" @click="closeDropdowns">
      ...
      <!-- Loading -->
      <div v-if="loadingEval" class="flex justify-center py-24">
      ...
      <!-- Error -->
      <div v-else-if="errorEval" class="mt-8 rounded-md bg-red-50 dark:bg-red-900/20 p-4 text-sm text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800">
        ⚠️ {{ errorEval }}
      </div>

      <!-- Contenido principal -->
      <template v-else-if="trabajoActual">
      <!-- Header y Regresar -->
      <div
      ...
      >
        <div class="flex items-start gap-4">
          ...
          <div class="flex flex-col gap-2">
            <h1
              class="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white flex items-center gap-3"
            >
              <span
                class="inline-flex items-center justify-center rounded-lg p-2 flex-shrink-0"
              :class="
                  trabajoActual.tipo === 'Examen'
                    ? 'bg-rose-100 text-rose-600 dark:bg-rose-900/50 dark:text-rose-400'
                    : trabajoActual.tipo === 'Tarea'
                      ? 'bg-purple-100 text-purple-600 dark:bg-purple-900/50 dark:text-purple-400'
                      : 'bg-blue-100 text-blue-600 dark:bg-blue-900/50 dark:text-blue-400'
                "
              >
                <svg
                  v-if="trabajoActual.tipo === 'Examen'"
                  class="w-6 h-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
                ...
                <svg
                  v-else-if="trabajoActual.tipo === 'Tarea'"
                  class="w-6 h-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
            ...
            <div
              class="mt-1 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 lg:gap-5"
            >
              <!-- Ponderación -->
              <div class="inline-flex">
                <span class="inline-flex items-center rounded-md bg-purple-50 px-2.5 py-1 text-xs font-medium text-purple-700 ring-1 ring-inset ring-purple-700/10 dark:bg-purple-900/40 dark:text-purple-300 dark:ring-purple-700/30">
                  Ponderación: {{ trabajoActual.ponderacion_mes ? `${trabajoActual.ponderacion_mes}% del Mes` : 'Automática (Restante)' }}
                </span>
              </div>
              ...
              <div class="flex flex-row items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                <div class="flex items-center gap-1.5">
                  ...
                  Inicio: <span class="text-gray-900 dark:text-gray-200 font-medium whitespace-nowrap">{{ trabajoActual.fecha_inicio }}</span>
                </div>
                <div class="flex items-center gap-1.5">
                  <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
                  </svg>
                  Entrega: <span class="text-gray-900 dark:text-gray-200 font-medium whitespace-nowrap">{{ trabajoActual.fecha_entrega }}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              >
                <thead class="bg-gray-50 dark:bg-gray-800/50">
                  <tr>
                    <th
                      scope="col"
                      class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-200 sm:pl-6 sticky left-0 z-10 bg-gray-50 dark:bg-gray-800 min-w-[250px] shadow-[1px_0_0_0_rgba(229,231,235,1)] dark:shadow-[1px_0_0_0_rgba(55,65,81,1)]"
                    >
                      Estudiante
                    </th>
                    ...
                    >
                      <div class="flex flex-col items-center">
                        <span
                          ...
                          :title="criterio.nombre"
                          >{{ criterio.nombre }}</span
                        >
                        <span class="text-xs font-normal text-gray-500 mt-0.5"
                          >Valdrá {{ criterio.porcentaje }}%</span
                        >
                      </div>
                    </th>
                    ...
                    <th
                      scope="col"
                      class="px-3 py-3.5 text-center text-sm font-bold text-brand-600 dark:text-brand-400 w-24 border-l border-gray-200 dark:border-gray-700 bg-brand-50/50 dark:bg-brand-900/10"
                    >
                      Total
                    </th>
                ...
                >
                  <tr
                    v-for="student in alumnos"
                    :key="student.id"
                    class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                  >
                    <!-- 1. Alumno Info -->
                    <td
                      class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6 sticky left-0 z-10 bg-white dark:bg-gray-900 shadow-[1px_0_0_0_rgba(229,231,235,1)] dark:shadow-[1px_0_0_0_rgba(55,65,81,1)]"
                    >
                      <div class="flex items-center">
                        <div
                          class="h-10 w-10 flex-shrink-0 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400 font-medium text-base"
                        >
                    ...
                    >
                      <input
                        ...
                        :disabled="!canEdit"
                        placeholder="-"
                        class="w-16 mx-auto rounded-md border-0 py-1.5 text-center text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-brand-600 sm:text-sm sm:leading-6 dark:bg-gray-800 dark:text-white dark:ring-gray-600 [&::-webkit-inner-spin-button]:appearance-none disabled:opacity-50 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
                      />
                    </td>

                    <!-- 3. Acumulado / Score (calculado sumando la multiplicación del peso) -->
                    <td
                      class="whitespace-nowrap px-3 py-4 text-sm text-center font-bold text-gray-900 dark:text-white border-l border-gray-200 dark:border-gray-700 bg-brand-50/20 dark:bg-brand-900/5"
                    >
                      {{ getNotaFinalAlumno(student.id) }}
                    ...
                    <!-- 4. Acciones: Emoji -->
                    <td
                      class="whitespace-nowrap px-3 py-4 text-sm text-center relative"
                    >
                      <button
                        :disabled="!canEdit"
                        @click.stop="toggleEmojiDropdown(student.id)"
                        class="h-8 w-8 rounded-full inline-flex items-center justify-center text-2xl transition-colors mx-auto hover:bg-gray-100 dark:hover:bg-gray-700 disabled:cursor-default disabled:hover:bg-transparent dark:disabled:hover:bg-transparent"
                      >
                      ...
                      <div
                        ...
                        class="absolute z-50 top-1/2 -translate-y-1/2 rounded-xl bg-white shadow-xl ring-1 ring-black ring-opacity-5 dark:bg-gray-700 dark:ring-gray-600 p-2 w-[152px]"
                    ...
                    <td
                      class="whitespace-nowrap py-4 pr-4 pl-3 sm:pr-6 text-center text-sm"
                    >
                      <button
                        ...
                        :class="
                          observacionesMap[student.id]
                            ? 'text-brand-600 bg-brand-50 hover:bg-brand-100 dark:bg-brand-900/30 dark:hover:bg-brand-900/50'
                      ...
                      >
                        ...
                        <span
                          v-if="observacionesMap[student.id]"
      ...
      <!-- Comentario General del Grupo -->
      <div
        class="mt-6 bg-white dark:bg-gray-900 shadow-sm border border-gray-200 dark:border-gray-800 rounded-xl px-6 py-6 mb-8"
      >
        <div>
          <h3
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-3xl">
            Opcional. Agrega observaciones, incidentes o notas globales sobre el
            desempeño del grupo en esta evaluación.
          </p>
        </div>
        <div class="mt-4">
          <textarea
            ...
            rows="3"
            :readonly="!canEdit"
            class="block w-full rounded-md border-0 py-2.5 px-4 text-gray-900 dark:text-white dark:bg-gray-800 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-700 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-brand-600 sm:text-sm sm:leading-6 resize-y read-only:opacity-60 read-only:cursor-not-allowed"
            placeholder="Escribe aquí tu comentario general..."
          ></textarea>
        </div>
      </div>
      </template><!-- /v-else-if trabajoActual -->
    ...
    <Teleport to="body">
    <div
      v-if="observacionVisible"
      class="fixed inset-0 z-[999999] flex items-end sm:items-center justify-center p-4"
      @click.self="cerrarModalObservacion"
    >
      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="cerrarModalObservacion" />
        <div
          class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0"
        >
          <div
            class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg dark:bg-gray-800"
          >
            <div
            ...
            >
              <div class="sm:flex sm:items-start">
                <div
                  class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-blue-100 sm:mx-0 sm:h-10 sm:w-10 dark:bg-blue-900/30"
                >
                  <svg
                ...
                >
                  ...
                  <div class="mt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">
                      Añade un comentario privado o feedback para el alumno
                    ...
                    <textarea
                      ...
                      rows="4"
                      :readonly="!canEdit"
                      class="block w-full rounded-md border-0 py-2.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-brand-600 sm:text-sm sm:leading-6 dark:bg-gray-700 dark:text-white dark:ring-gray-600 px-3 read-only:opacity-60 read-only:cursor-not-allowed"
            ...
            <div
              class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 dark:bg-gray-800/80 border-t border-gray-200 dark:border-gray-700"
            >
              <button
                v-if="canEdit"
                type="button"
